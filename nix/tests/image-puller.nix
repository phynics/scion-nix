...
pkgs.runCommand "scion-image-puller-${arch}-check" {
  ...
  output=$(PATH="${runtime}/bin:$PATH" ${puller}/bin/scion-pull-images podman)
  test "$(printf '%s\n' "$output" | wc -l)" -eq 18
  printf '%s\n' "$output" | grep -F "pull ghcr.io/phynics/scion-claude@${claude.digest}" > /dev/null
  printf '%s\n' "$output" | grep -F "tag ghcr.io/phynics/scion-claude@${claude.digest} ghcr.io/phynics/scion-claude:latest" > /dev/null
